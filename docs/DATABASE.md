...
- **Eighth migration (STORY-010)**: `5354c755424b_add_workflow_event_sequence_and_tool_.py`
  (`down_revision = "291a26a9ce1b"`) makes two changes to
  `workflow_events`: adds `sequence` (`BIGINT`, `GENERATED BY DEFAULT AS
  IDENTITY`, `UNIQUE`, plus a supporting `(workflow_run_id, sequence)`
  index) as the new AUTHORITATIVE ordering key for reading a workflow's
  event history, and extends the `workflow_event_type` `CHECK`
  constraint with one new value, `tool_invoked` — see
  [WORKFLOWS.md](WORKFLOWS.md) Section 5 "Ordering" and
  [AI_SAFETY.md](AI_SAFETY.md). The `sequence` column fixes a genuine
  flaky-ordering bug (two events created in rapid succession — exactly
  what a tool-call event sequence produces — could previously tie on
  `created_at` at Python timestamp resolution, with a random UUID `id`
  as an ineffective tiebreaker) discovered during this story's own
  mandatory pre-check baseline run, not a hypothetical concern. The
  `CHECK` constraint change is applied by dropping and recreating it
  under its ORIGINAL (short, naming-convention-templated) name, so the
  final constraint name is identical to what the model itself would
  produce on a fresh bootstrap. Validated against real PostgreSQL:
  applied, inspected (`sequence` present as `bigint generated by
  default as identity`, `tool_invoked` present in the `CHECK`
  constraint's allowed values), downgraded one step (confirming the
...
- **`WorkflowRun`/`WorkflowStep`/`WorkflowEvent` model/repository/service/
  ...
  would.
- **AI/tool-calling tests (STORY-010)** (`backend/tests/ai/`,
  `backend/tests/api/test_agent_endpoints.py`) — decision-schema and
  safety-policy tests (`test_decisions.py`, `test_safety.py`) need no
  database at all and run unconditionally; provider tests
  (`test_providers.py`) construct a real `AnthropicProvider` instance
  with a synthetic key but make no network call, also unconditional;
  tool, orchestration, and API tests
  (`test_tools.py`, `test_orchestration.py`,
  `test_agent_endpoints.py`) run against real PostgreSQL under the same
  savepoint isolation, using `FakeLLMProvider`
  (`app.ai.providers.fake_provider`) instead of a real Anthropic call —
  never requiring internet access or a real `LLM_API_KEY`. The
  mandatory full end-to-end proof
  (`test_agent_endpoints.py::test_full_chain_patient_request_to_persisted_appointment_and_workflow`)
  exercises the complete chain — authenticated PATIENT -> HTTP request
  -> fake structured decision -> tool registry -> real
  `AppointmentService` -> real PostgreSQL row -> `WorkflowRun`/
...
- This story also fixed a genuine flaky-ordering bug it discovered in
  ...
  fail when two synthetic rows were created back-to-back within the
  same Python timestamp-resolution window. Both tests were hardened
  with an explicit timestamp gap (no production behavior changed for
  either); `WorkflowEvent`'s own ordering was fixed at the SCHEMA level
  instead (the new `sequence` column, Section 8's eighth migration),
  since STORY-010's own audit trail genuinely creates several events in
  rapid succession per request.

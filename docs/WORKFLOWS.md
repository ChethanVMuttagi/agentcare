...
(`app/api/v1/endpoints/workflows.py`), extended in STORY-010 (the
`tool_invoked` event type, the `sequence` ordering column, and
`AgentOrchestrationService`'s integration with this layer — see
[AI_SAFETY.md](AI_SAFETY.md) and [TOOLS.md](TOOLS.md)). It follows the
...
today; anything marked PLANNED does not yet. See
...
| `agent_name` | VARCHAR(100) | NULLABLE — used meaningfully as of STORY-011: `"coordinator"`/`"scheduling"`/`"document"`/`"routing"` — see [AGENTS.md](AGENTS.md) Section 6, and Section 8 below |
...
| `sequence` | BIGINT | NOT NULL, UNIQUE, `GENERATED BY DEFAULT AS IDENTITY` — see "Ordering" below |

`WorkflowEventType` (`enum.StrEnum`): `workflow_created`,
`workflow_started`, `step_started`, `tool_invoked`, `agent_handoff`,
`step_completed`, `step_failed`, `step_skipped`, `workflow_waiting`,
`workflow_resumed`, `workflow_completed`, `workflow_failed`,
`workflow_cancelled`. (`step_skipped` was one addition beyond the
story's suggested list in STORY-009, added for symmetry with
`StepStatus.SKIPPED` — see Section 7. `tool_invoked` was added in
STORY-010 — see [AI_SAFETY.md](AI_SAFETY.md) and
[adr/ADR-0010-llm-and-tool-security-boundary.md](adr/ADR-0010-llm-and-tool-security-boundary.md).
`agent_handoff` was added in STORY-011 (migration `b5456329cbdd`),
recording a Coordinator agent handing a request off to one specialist —
`safe_metadata` carries only `{"from_agent": ..., "to_agent": ...}` —
see [AGENTS.md](AGENTS.md) Section 6.)

### Ordering

`list_by_run` orders by `sequence`, NOT `created_at` (STORY-010 fix,
migration `5354c755424b`). `created_at` is Python-generated
(`datetime.now(UTC)`), and two events created in rapid succession — the
exact shape a tool-call event sequence produces
(`step_started` → `tool_invoked` → `step_completed`, all within one
request) — can share the same value at timestamp resolution; a random
UUID `id` is not a valid tiebreaker for insertion order. `sequence` is
a PostgreSQL `GENERATED BY DEFAULT AS IDENTITY` column: server-assigned,
strictly monotonically increasing, and immune to both problems by
construction — concurrent inserts can never receive the same value, and
its order always matches true insertion order. This bug was discovered
during STORY-010's own mandatory pre-check baseline run (a flaky test
failure) and fixed in the same migration STORY-010 already needed for
`tool_invoked` — see
[adr/ADR-0010-llm-and-tool-security-boundary.md](adr/ADR-0010-llm-and-tool-security-boundary.md)
"Rationale."
...
`ActorType` (`enum.StrEnum`): `user`, `system`, `agent`, `tool` — added
in STORY-009 preparing for non-human actors without a later schema
change; `AGENT`/`TOOL` became genuinely populated starting in STORY-010
(`tool_invoked` events) and, for `AGENT`, are now used across BOTH the
coordination and specialist-execution steps as of STORY-011.
...
- `ActorType.AGENT`: the stable agent name that performed the action —
  `"coordinator"`, `"scheduling"`, `"document"`, or `"routing"` (STORY-011)
  — never a provider or model name, never a serialized prompt.
- `ActorType.TOOL`: the tool's own name, e.g. `"book_appointment"` —
  never a serialized tool payload.

`WorkflowStep.agent_name` (≤ 100 chars) holds the same stable agent
names as above, set on every step since STORY-011 — see
[AGENTS.md](AGENTS.md) Section 6. `WorkflowStep.tool_name` remains
unused by application code as of this story (no step is currently
created per-tool; a tool's name is recorded via the `tool_invoked`
event's `safe_metadata` instead — see [AI_SAFETY.md](AI_SAFETY.md)
Section 10).
...
workflow_events
  Check constraints:
      ck_workflow_events_workflow_event_type          (includes tool_invoked as of STORY-010)
  ...
  Unique constraints:
      uq_workflow_events_sequence                     (STORY-010 — see "Ordering" in Section 5)
```
...
(Sections 9 and 17). STORY-010's migration (`5354c755424b`, adding
`sequence` and `tool_invoked`) was validated the same way: applied,
inspected, downgraded one step (confirming all eleven prior tables plus
STORY-009's original three-table shape survive untouched), and
re-upgraded.
...
**Realized in STORY-010**: `app.ai.orchestration.AgentOrchestrationService`
is exactly this "future agent/orchestration process" — it calls
`WorkflowService.create_workflow`/`start_workflow`/`create_step`/
`start_step`/`record_tool_invocation`/`complete_step`/`fail_step`/
`complete_workflow`/`fail_workflow` directly, in-process, from
`POST /api/v1/organizations/{organization_id}/agent/execute` — a
SEPARATE route (see [AI_SAFETY.md](AI_SAFETY.md) and
[TOOLS.md](TOOLS.md)), not an addition to this router. The
`workflows` API above remains inspection/cancellation only.
...
**Upheld in STORY-010**: the first LLM-integration story that actually
arrived kept this constraint exactly as anticipated —
`AgentExecuteRequest.request_text` is never persisted anywhere in the
`WorkflowRun`/`WorkflowStep`/`WorkflowEvent` chain (proven directly in
`tests/ai/test_orchestration.py::test_no_raw_request_text_is_ever_persisted`).
See [AI_SAFETY.md](AI_SAFETY.md) for the full LLM trust boundary and
healthcare safety policy this integration adds.

## 24. Current vs. Planned

**Current (STORY-009):** `WorkflowRun`, `WorkflowStep`, `WorkflowEvent`
...
**Current (STORY-010, additive):** a provider-independent `LLMProvider`
abstraction and one real provider (Anthropic Claude); a structured,
schema-validated `AdministrativeDecision` contract with no
chain-of-thought, enforced structurally; a deterministic healthcare
safety pre-screen; an explicit, allowlisted `ToolRegistry` with two
real tools (`check_availability`, `book_appointment`) calling the real
service layer; the `tool_invoked` event type and the `sequence`
ordering column (migration `5354c755424b`); full integration wiring
`AgentOrchestrationService` to `WorkflowService` for every step of the
one-decision/at-most-one-tool-execution flow — see
[AI_SAFETY.md](AI_SAFETY.md) and [TOOLS.md](TOOLS.md).

**Current (STORY-011, additive):** genuine multi-agent coordination — a
Coordinator agent and three specialists (Scheduling, Document, Routing),
each with a separate, application-code-enforced tool allowlist; the
`agent_handoff` event type (migration `b5456329cbdd`); a two-step
workflow shape (coordination, then specialist execution) for a
successful handoff; `WorkflowStep.agent_name` used meaningfully for the
first time. Still "at most one Coordinator decision, at most one
...
**Explicitly not implemented yet** (later stories): unrestricted
multi-step planning, recursive or specialist-to-specialist delegation, a
LangGraph graph, autonomous loops, medical reasoning/diagnosis/
treatment/prescription/dosage advice of any kind, background workers/
...
log (Section 18), reschedule/cancel tools (see [TOOLS.md](TOOLS.md)
Section 6), a frontend.

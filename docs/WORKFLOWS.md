...
`workflow_started`, `step_started`, `tool_invoked`, `step_completed`,
`step_failed`, `step_skipped`, `workflow_waiting`, `workflow_resumed`,
`workflow_completed`, `workflow_failed`, `workflow_cancelled`.
(`step_skipped` was one addition beyond the story's suggested list in
STORY-009, added for symmetry with `StepStatus.SKIPPED` — see Section
7. `tool_invoked` was added in STORY-010 — see
[AI_SAFETY.md](AI_SAFETY.md) and
[adr/ADR-0010-llm-and-tool-security-boundary.md](adr/ADR-0010-llm-and-tool-security-boundary.md).)
...
**Explicitly not implemented yet** (later stories): the final
multi-agent architecture, agent-to-agent delegation, a LangGraph graph,
autonomous multi-step planning loops, medical reasoning/diagnosis/
